...
const ProgramacionPersonalizada = ({ 
  ...
  const generarHorarioUnico = (horariosDia = []) => {
    const inicioMinutos = 9 * 60; // 09:00 por defecto

    for (let offset = 0; offset < 24; offset++) {
      const totalMinutos = (inicioMinutos + (offset * 60)) % (24 * 60);
      const hora = Math.floor(totalMinutos / 60);
      const minutos = totalMinutos % 60;
      const horario = `${String(hora).padStart(2, '0')}:${String(minutos).padStart(2, '0')}`;
      if (!horariosDia.includes(horario)) {
        return horario;
      }
    }

    // Fallback improbable si ya están ocupadas todas las horas en punto
    return '09:00';
  };
  ...
  const agregarToma = (diaId) => {
    ...
    const nuevoHorario = generarHorarioUnico(nuevoProgramacion[diaId]);
