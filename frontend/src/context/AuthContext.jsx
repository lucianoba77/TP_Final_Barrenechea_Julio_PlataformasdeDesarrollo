...
export const AuthProvider = ({ children }) => {
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      // Si es un redirect, no esperamos resultado aquí
      // El resultado se manejará cuando el usuario vuelva de Google
      if (resultado.redirecting) {
        // La página será redirigida, no necesitamos hacer nada más
        return resultado;
      }
  ...
  // Verificar si el usuario es asistente al cargar (solo con token para evitar 401 y errores en consola)
  useEffect(() => {
    const verificarRolAsistente = async () => {
      if (!usuarioActual?.email || usuarioActual.role || !hasToken()) return;
