...
const DashboardScreen = () => {
  ...
  const medicamentosHoy = medicamentos.filter(medicamento => {
    ...
    if ((Number(medicamento.stockActual) || 0) <= 0) return false;
    if (esMedicamentoOcasional(medicamento)) return false;
  ...
  // Monitorear stock y mostrar alertas solo para medicamentos que se muestran hoy en dashboard
  useStockAlerts(
    preferenciasAlerta.diasAntesStock,
    preferenciasAlerta.notificacionesActivas,
    medicamentosHoy
  );
