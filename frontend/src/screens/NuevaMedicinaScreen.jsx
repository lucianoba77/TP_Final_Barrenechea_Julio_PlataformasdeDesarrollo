...
const normalizarHorariosTomas = (horarios = []) => {
...
const construirHorariosPorDefecto = (cantidad, horaBase = '08:00', horariosExistentes = []) => {
  const cantidadNumerica = Math.max(0, Number(cantidad) || 0);
  if (cantidadNumerica === 0) return [];

  const baseValida = REGEX_HORA.test(String(horaBase || '')) ? String(horaBase) : '08:00';
  const [hora, minutos] = baseValida.split(':').map(Number);
  const intervalo = 24 / Math.max(1, cantidadNumerica);

  const horarios = Array.isArray(horariosExistentes) ? [...horariosExistentes] : [];
  if (!REGEX_HORA.test(horarios[0] || '')) {
    horarios[0] = baseValida;
  }

  for (let i = horarios.length; i < cantidadNumerica; i++) {
    const horasToma = hora + (intervalo * i);
    const horaToma = Math.floor(horasToma) % 24;
    horarios[i] = `${String(horaToma).padStart(2, '0')}:${String(minutos).padStart(2, '0')}`;
  }

  return horarios.slice(0, cantidadNumerica);
};
...
const NuevaMedicinaScreen = () => {
        ...
        try {
          ...
          if (resultado.success && resultado.medicamento) {
            ...
            setFormData({
              ...
              horariosTomas: Array.isArray(med.horariosTomas) && med.horariosTomas.length > 0
                ? med.horariosTomas
                : construirHorariosPorDefecto(med.tomasDiarias || 1, med.primeraToma || '08:00')
            });
  ...
  const cambioCampoFormulario = (e) => {
    ...
    setFormData(prev => {
      ...
      if (name === 'tomasDiarias' && !nuevoEstado.usarProgramacionPersonalizada) {
        nuevoEstado.horariosTomas = construirHorariosPorDefecto(
          nuevoValor,
          nuevoEstado.horariosTomas?.[0] || nuevoEstado.primeraToma || '08:00',
          nuevoEstado.horariosTomas
        );
        nuevoEstado.primeraToma = nuevoEstado.horariosTomas[0] || '';
      }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const esOcasionalPorRegla = !formData.esCronico && (Number(formData.diasTratamiento) || 0) === 0;
    ...
    if (!esOcasionalPorRegla && formData.usarProgramacionPersonalizada) {
    ...
    } else if (!esOcasionalPorRegla && formData.tomasDiarias > 0) {
      const horariosValidos = (formData.horariosTomas || []).filter((horario) => REGEX_HORA.test(String(horario || '')));
      if (horariosValidos.length !== Number(formData.tomasDiarias)) {
        showError('Debes configurar el horario de todas las tomas diarias');
        return;
      }
      if (new Set(horariosValidos).size !== horariosValidos.length) {
        showError('No puedes repetir horarios en las tomas diarias');
        return;
      }
      if (!formData.esCronico && (!formData.diasTratamiento || formData.diasTratamiento <= 0)) {
        showError('Ingresa la cantidad de días de tratamiento');
    ...
    const formDataNormalizado = {
      ...formData,
      tomasDiarias: esOcasionalPorRegla ? 0 : formData.tomasDiarias,
      usarProgramacionPersonalizada: esOcasionalPorRegla ? false : formData.usarProgramacionPersonalizada,
      programacionPersonalizada: esOcasionalPorRegla ? null : formData.programacionPersonalizada,
      horariosTomas: esOcasionalPorRegla
        ? []
        : (
          formData.usarProgramacionPersonalizada
            ? []
            : normalizarHorariosTomas(formData.horariosTomas)
        ),
    };
    formDataNormalizado.primeraToma = formDataNormalizado.horariosTomas[0] || '';

    if (esEdicion && medicamentoIdEditar) {
      // Modo edición
      const resultado = await editarMedicina(medicamentoIdEditar, formDataNormalizado);
  ...
  const primeraTomaDetectada =
    !formData.usarProgramacionPersonalizada && Number(formData.tomasDiarias) > 0
      ? normalizarHorariosTomas(formData.horariosTomas)[0] || null
      : null;
    ...
    <div className="nueva-medicina-screen">
      ...
      <div className="nm-container">
        ...
        <form onSubmit={handleSubmit} className="nm-form">
          ...
          <div className="form-group-row">
            ...
            <div className="form-group">
              <label htmlFor="primeraToma">
                Horario de toma #1 {formData.tomasDiarias === 0 ? '(opcional)' : '*'}
              </label>
              <input
                type="time"
                id="primeraToma"
                name="primeraToma"
                value={formData.horariosTomas?.[0] || ''}
                onChange={(e) => {
                  const nuevoHorario = e.target.value;
                  setFormData(prev => {
                    const horarios = Array.isArray(prev.horariosTomas) ? [...prev.horariosTomas] : [];
                    horarios[0] = nuevoHorario;
                    return {
                      ...prev,
                      primeraToma: nuevoHorario,
                      horariosTomas: horarios
                    };
                  });
                }}
          ...
          {!formData.usarProgramacionPersonalizada && formData.tomasDiarias > 1 && (
            <div className="form-group multi-horarios-section">
              <label>Horarios de tomas</label>
              {Array.from({ length: Number(formData.tomasDiarias) - 1 }, (_, idx) => {
                const indiceHorario = idx + 1;
                return (
                  <div key={indiceHorario} className="form-group-row" style={{ marginTop: '8px' }}>
                    <div className="form-group" style={{ width: '100%' }}>
                      <label htmlFor={`horarioToma${indiceHorario + 1}`}>Toma #{indiceHorario + 1}</label>
                      <input
                        type="time"
                        id={`horarioToma${indiceHorario + 1}`}
                        value={formData.horariosTomas?.[indiceHorario] || ''}
                        onChange={(e) => {
                          const nuevoHorario = e.target.value;
                          setFormData(prev => {
                            const horarios = Array.isArray(prev.horariosTomas) ? [...prev.horariosTomas] : [];
                            horarios[indiceHorario] = nuevoHorario;
                            return {
                              ...prev,
                              horariosTomas: horarios
                            };
              ...
              <p className="helper-text">
                Estos horarios se repetirán todos los días mientras no actives la programación personalizada.
              </p>
              <p className="helper-text">
                Puedes ingresarlos en cualquier orden (ej: noche y luego mañana).
              </p>
              {primeraTomaDetectada && (
                <div className="first-dose-hint">
                  Primera toma del día detectada: <strong>{primeraTomaDetectada}</strong>
                </div>
              )}
            </div>
          )}
                ...
                <input
                  ...
                  onChange={(e) => {
                    setFormData(prev => ({
                      ...
                      horariosTomas: e.target.checked
                        ? []
                        : construirHorariosPorDefecto(prev.tomasDiarias, prev.horariosTomas?.[0] || prev.primeraToma || '08:00', prev.horariosTomas),
                      primeraToma: e.target.checked
                        ? ''
                        : (prev.horariosTomas?.[0] || prev.primeraToma || '08:00')
                    }));
                  }}
          ...
          <div className="form-group-row">
            <div className="form-group">
              <label htmlFor="stockInicial">{esEdicion ? 'Stock actual (restante)' : 'Stock inicial'}</label>
              <input
                type="number"
                id="stockInicial"
                name="stockInicial"
              ...
              {esEdicion && (
                <p className="helper-text">
                  Este valor muestra el stock disponible actual, descontando tomas ya registradas.
                </p>
              )}
            </div>
            ...
            <div className="form-group">
              ...
              {formData.tomasDiarias > 0 && !formData.esCronico && Number(formData.diasTratamiento) === 0 && (
                <p className="helper-text">
                  Con 0 días y no crónico, se guardará como medicamento ocasional (sin tomas programadas).
                </p>
              )}
              <button
                type="button"
